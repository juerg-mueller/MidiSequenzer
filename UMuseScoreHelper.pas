...
type
  TMSHelper = class helper for TGriffPartitur
    function LoadFromMscx(const FileName: string): boolean;
...
uses
  UXmlNode, UXmlParser, UEventArray, UGriffEvent, UGriffArray,
  UMyMidiStream, UMyMemoryStream, UInstrument;

function TMSHelper.LoadFromMscx(const FileName: string): boolean;
const
...
var
  ...
  begin
    result := rRegular;
    d.Left := GriffHeader.Details.TicksPerMeasure * (offset_ div GriffHeader.Details.TicksPerMeasure);
    d.Right := d.Left + GriffHeader.Details.TicksPerMeasure;
...
{$else}
                        ...
                        begin
                          ...
                          s := Note.Value;
                          if s = 'whole' then
                            duration := 4*GriffHeader.Details.DeltaTimeTicks
                          else
                          if s = 'half' then
                            duration := 2*GriffHeader.Details.DeltaTimeTicks
                          else
                          if s = 'quarter' then
                            duration := GriffHeader.Details.DeltaTimeTicks
                          else
                          if s = 'eighth' then
                            duration := GriffHeader.Details.DeltaTimeTicks div 2
                          else
                          if s = '16th' then
                            duration := GriffHeader.Details.DeltaTimeTicks div 4
                          else
                          if s = '32nd' then
                            duration := GriffHeader.Details.DeltaTimeTicks div 8
                          else
                          if s = '64th' then
                            duration := GriffHeader.Details.DeltaTimeTicks div 16
                          else
                          if s = '128th' then
                          begin
                            duration := GriffHeader.Details.DeltaTimeTicks div 32;
                       {$if defined(CONSOLE)}
                          end else
                          if s <> 'measure' then
                            writeln('error: ', s);
...
function TMSHelper.SaveToMscx(const FileName: string): boolean;
const
  UseBellows = true;
  UseColors = false;
  UseGrandCross = true;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (iEvent < SaveRec.iEnd) and
           (GriffEvent.GriffPitch = GriffEvents[iEvent+1].GriffPitch) then
        begin
    ...
    begin
      ...
      Child1 := Child.AppendChildNode('name', 'noteheadXBlack');
    ...
    if GriffEvent.GriffPitch <> GriffEvent.SoundPitch then
      Child := Child.AppendChildNode('pitch',
        IntToStr(integer(GriffEvent.SoundPitch) - GriffEvent.GriffPitch));
    Child := NoteNode.AppendChildNode('pitch', IntToStr(GriffEvent.GriffPitch));
    Child := NoteNode.AppendChildNode('tpc', IntToStr(MuseScoreTPC[GriffEvent.GriffPitch mod 12]));
  ...
  begin
    ...
    m := mes div GriffHeader.Details.TicksPerMeasure;
    if (mes mod GriffHeader.Details.TicksPerMeasure) > 0 then
      inc(m);
    m := n*m;

    Child := next.AppendChildNode('location');
    Child := Child.AppendChildNode('measures', IntToStr(m));
  end;
  ...
  begin
    ...
    if not visible then
    begin
      Child1 := Child.AppendChildNode('visible', '0');
    end;
    Child1 := Child.AppendChildNode('endHookType', '1');
    Child1 := Child.AppendChildNode('beginText', s + '.');
    Child1 := Child.AppendChildNode('endings', s);
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Child2 := Child1.AppendChildNode('subtype' , 'start-repeat');
            Child2 := Child1.AppendChildNode('visible', '0');
          end else begin
        ...
        if (LastRepeat = rVolta2Stop) and (Volta2Off >= 0) then
        begin
  ...
  begin
    ...
    if not Visible then
    begin
      Child1 := result.AppendChildNode('visible', '0');
    end;
    Child1 := result.AppendChildNode('sigN', IntToStr(GriffHeader.Details.measureFact));
    Child1 := result.AppendChildNode('sigD', IntToStr(GriffHeader.Details.measureDiv));
  end;
{$ifndef OldStyle}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SaveRec.sLen := SaveRec.GetLenS(t, quarterNote);
      end;
      ...
      inc(SaveRec.offset, quarterNote*(t1 - t) div 8 );
      NeuerTakt(Visible);
    end;
  end;
{$else}
  procedure AddRest(Len: integer; Lyrics, Visible: boolean);
  var
    t, t1: integer;
  ...
  begin
    ...
    begin
      t1 := t;
      RestNode := VoiceNode.AddChild('Rest');
      if Lyrics then
      begin
        Child := RestNode.AddChild('offset');
        Child.Attributes['x'] := '0';
        Child.Attributes['y'] := '-4';
      end;
      sLen := GetLen(t, dot, SaveRec.t32takt);
      if dot then
        RestNode.AddChild('dots', '1');
      RestNode.AddChild('durationType', sLen);
      inc(SaveRec.t32takt, t1 - t);
    end;
    inc(SaveRec.offset, Len);
    NeuerTakt(false);
  end;
{$endif}
  ...
  procedure AppendStaff(Lyrics: boolean; nt: TNoteType);
    ...
    begin
      ...
      begin
        if not Next then
          s := '-' + s;
        Child1 := Child1.AppendChildNode('fractions', s);
      end else begin
        if Next then
          s := '1'
        else
          s := '-1';
        Child1 := Child1.AppendChildNode('measures', s);
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        if Dot then
        begin
          Child1 := ChordNode.AppendChildNode('dots', '1');
        end;
        Child := ChordNode.AppendChildNode('durationType', sLen);
        if (sLen = 'eighth') and
           (nt = ntBass) and WithBass then
        begin
          Child1 := ChordNode.AppendChildNode('Articulation');
          Child2 := Child1.AppendChildNode('subtype');
          if GriffEvents[iEnd].SoundPitch >= 45 then
  ...
  var
    i: integer;
    NoteNode, Child, Child1, Child2, Child3: KXmlNode;
    Aufrunden: boolean;
  begin
    Aufrunden := false;
    if Lyrics then
      Aufrunden := AufViertelnotenAufrunden;
    with SaveRec do
    begin
      Clear;
      Volta1Off := -1;
...
//      PushStartOffset := -1;
      ...
      begin
        ...
        Child1 := Child.AppendChildNode('tempo',
          Format('%f', [GriffHeader.Details.beatsPerMin / 60.0]));
        Child1 := Child.AppendChildNode('followText', '1');
      ...
      //  if NoteType in [ntDiskant, ntBass] then
        begin
          ...
          // Push-Striche
          if (NoteType = ntDiskant) {or Instrument.BassDiatonic} then
            if (InPu <> InPush) then
            begin
              //ShowBellows(InPush);
              InPu := InPush;
          ...
          begin
            ...
            begin
              ...
              Child2 := Child1.AppendChildNode('normalNotes', '2');
              Child2 := Child1.AppendChildNode('actualNotes', '3');
              Child2 := Child1.AppendChildNode('baseNote', sLen);
              Child2 := Child1.AppendChildNode('Number');
              Child3 := Child2.AppendChildNode('style', 'Tuplet');
              Child3 := Child2.AppendChildNode('text', '3');
              for i := 1 to 3 do
              begin
                ChordNode := VoiceNode.AppendChildNode('Chord');
                Child1 := ChordNode.AppendChildNode('durationType', sLen);
            ...
            begin
              ...
              begin
                i := iEvent;
                if (nt = ntDiskant) and
                   self.IsAppoggiatura(SaveRec) then
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        Child1 := Child.AppendChildNode('size', '14');  // 14 pt
                      end;
                      Child1 := Child.AppendChildNode('text', IntToStr(GriffEvents[iEvent].GriffPitch));

                      if (iEvent < iEnd) and
                         (GriffEvents[iEnd].NoteType = ntBass) then
                      begin
                        Child := VoiceNode.AppendChildNode('StaffText');
                        Child1 := Child.AppendChildNode('text', IntToStr(GriffEvents[iEnd].GriffPitch));
                      end;
                    end else begin
                      Child := VoiceNode.AppendChildNode('StaffText');
                      if not GriffEvents[iEvent].InPush then
                        AddHeadColor(Child);
                      Child1 := Child.AppendChildNode('text', GriffEvents[iEvent].GetSteiBass);
                      if (iEvent < iEnd) and
                         (GriffEvents[iEnd].NoteType = ntBass) then
                      begin
                        Child := VoiceNode.AppendChildNode('StaffText');
                        if not GriffEvents[iEvent].InPush then
                          AddHeadColor(Child);
                        Child1 := Child.AppendChildNode('text', GriffEvents[iEnd].GetSteiBass);
                      end;
                    end;
                  end;
                  ...
                  if Dot then
                  begin
                    Child1 := Child.AppendChildNode('dots', '1');
                  end;
                  Child1 := Child.AppendChildNode('durationType', sLen);
                end else begin
                  AddChord(true);
                  while i <= iEnd do
                    ...
                    begin
                      ...
                      begin
                          ...
                          case appoggiatura of
                            ...
                            tieStop:
                              begin
                                ...
                                Child1 := Child1.AppendChildNode('location');
                                Child1 := Child1.AppendChildNode('grace', '0');
                                appoggiatura := tieOff;
                              end;
                          end;
                        end;
                        ...
                        if not Lyrics then
                        begin
                          Child := NoteNode.AppendChildNode('pitch',
...
//      if InPu and (nt = ntDiskant) then
        //ShowBellows(false);

      SaveRec.Len := SaveRec.MostRight mod SaveRec.Takt;
      if Len > 0 then
        Len := MostRight + Takt - Len - Offset;
//      while (SaveRec.Rest(GriffHeader.Details.GetRaster(MostRight - Offset), false)) do
      if Len > 0 then
        AddRest(Len, Lyrics, nt = ntDiskant);

      if Length(VoiceNode.ChildNodes) >= 2 then
  ...
  var
    TextLine, Segment, Child1, Child2: KXmlNode;
    off, mes, fra, quot: integer;

    procedure AddSubtype(Sub: string; Y: string);
    begin
  ...
  begin
    ...
    begin
      ...
      if mes > 0 then
      begin
        Child2 := Child1.AppendChildNode('measures', IntToStr(mes));
      end;
      if fra > 0 then
      begin
        quot := 32;
        ...
        Child2 := Child1.AppendChildNode('fractions', IntToStr(fra) + '/' + IntToStr(quot));
      end;
  ...
  begin
    ...
    if Length(VoiceNode.ChildNodes) >= 2 then
      AddRest(SaveRec.Takt, false, false, true);
    StaffNode.RemoveChild(MeasureNode_);
  end;
  ...
  begin
    ...
    Child2 := Child1.AppendChildNode('name', 'stdNormal');

    if Invisual then
    begin
      Child2 := Child1.AppendChildNode('clef', '0');
      Child2 := Child1.AppendChildNode('barlines', '0');
      Child2 := Child1.AppendChildNode('timesig', '0');
      Child2 := Child1.AppendChildNode('invisible', '1');

      Child2 := Child.AppendChildNode('invisible', '1');
      Child2 := Child.AppendChildNode('hideSystemBarLine', '1');
    end else
    if BassClef then
    begin
      Child1 := Child.AppendChildNode('defaultClef', 'F');
    end;

    if BassClef then
    begin
      Child := result.AppendChildNode('show', '0');
    end;
    Child := result.AppendChildNode('trackName', 'Akkordeon');
    Child := result.AppendChildNode('Instrument');
    Child.AppendAttr('id', 'accordion');
    Child1 := Child.AppendChildNode('trackName', 'Akkordeon');
    Child1 := Child.AppendChildNode('instrumentId', 'keyboard.accordion');
    Child1 := Child.AppendChildNode('Articulation');
    Child2 := Child1.AppendChildNode('velocity', '100');
    Child2 := Child1.AppendChildNode('gateTime', '100');
    ...
    Child2 := Child1.AppendChildNode('synti', 'Fluid');
  ...
  procedure MergeStaff(var Staff1, Staff3: KXmlNode);
  var
    mea1, mea3, p: integer;
    Child: KXmlNode;
  begin
    mea1 := -1;
    mea3 := -1;
    while (mea1 < Staff1.Count) and (mea3 < Staff3.Count) do
    begin
      inc(mea1);
      while (mea1 < Staff1.Count) and (Staff1.ChildNodes[mea1].Name <> 'Measure') do
        inc(mea1);
      inc(mea3);
      while (mea3 < Staff3.Count) and (Staff3.ChildNodes[mea3].Name <> 'Measure') do
        inc(mea3);
      if (mea1 < Staff1.Count) and (mea3 < Staff3.Count) then
      begin
        Child := Staff3.ChildNodes[mea3]; // measure
        // startRepeat und endRepeat überspringen
        p := Length(Child.ChildNodes)-1;
        while (p > 0) and (Child.ChildNodes[p].Name <> 'voice') do
          dec(p);
        Staff1.ChildNodes[mea1].AppendChildNode(Child.ChildNodes[p]);
...
var
  Root, Score, Part, Child, Child1, Child2, Child3: KXmlNode;
...
begin
  WithBass := false;
  for p := 0 to UsedEvents-1 do
    if GriffEvents[p].NoteType = ntBass then
      WithBass := true;


  Root := NewXmlNode('museScore');
  Root.AppendAttr('version', '3.02');
  Child := Root.AppendChildNode('programVersion', '3.6.2');
  Child := Root.AppendChildNode('programRevision');
  ...
  Child := Score.AppendChildNode('currentLayer', '0');
  Child := Score.AppendChildNode('Division', IntToStr(quarterNote));

  Child := Score.AppendChildNode('Style');
{$ifndef OldStyle}
  Child.AppendChildNode('minSystemDistance', '13'); // Abstand zw. Notenlinien
  // Lyrics unten
...
{$endif}
  Child := Score.AppendChildNode('showInvisible', '0');
  Child := Score.AppendChildNode('showUnprintable', '1');
  Child := Score.AppendChildNode('showFrames', '1');
  Child := Score.AppendChildNode('showMargins', '1');
  ...
  if Copy(s, Length(s)-3) = '_cap' then
    SetLength(s, Length(s)-4);
  Child.Value := s;

  ///////////////////////////////////////////////////////////////// Score Part 1

  Part := AddPart(1, false, false, Score);
...
{$endif}
  ...
  Child3 := Child2.AppendChildNode('style', 'Title');
  t := s;
  p := system.Pos(' - ', s);
  if p > 1 then
    Delete(s, p, length(s));
  Child3 := Child2.AppendChildNode('text', s);
  ...
  begin
    ...
    Delete(s, 1, p + 2);
    Child2 := Child1.AppendChildNode('Text');
    Child3 := Child2.AppendChildNode('style', 'Subtitle');
    Child3 := Child2.AppendChildNode('text', s);
  end;
  Child2 := Child1.AppendChildNode('Text');
  Child3 := Child2.AppendChildNode('style', 'Composer');
  Child3 := Child2.AppendChildNode('text', Instrument.Name);
